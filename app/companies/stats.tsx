...
const stats = [
  {
    icon: <RiBuilding2Line className='size-8 text-gray-500' />,
    value: '8+',
    label: 'Flagship Brands',
    description: 'Leading industries from beauty to fashion.',
  },
  {
    icon: <RiUserStarLine className='size-8 text-gray-500' />,
    value: '100k+',
    label: 'Lives Touched',
    description: 'Through our various products and services.',
  },
  {
    icon: <RiCommunityLine className='size-8 text-gray-500' />,
    value: '#1',
    label: 'Movement',
      ...
      <div className='container mx-auto px-6'>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-12'>
          {stats.map((stat, idx) => (
            ...
            >
              <div className='p-4 rounded-2xl bg-muted'>{stat.icon}</div>
              ...
              <p className='text-gray-600 font-normal'>{stat.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
